...
if(isset($_POST['add'])) {
    ...
    $count = mysqli_num_rows(mysqli_query($conn, "SELECT categoryID FROM tbl_product_category"));
    $not_inserted = true;
    for($i = 0; $i <= $count; $i++) {
        $temp = "category_".$i;
        $res = mysqli_query($conn, "SELECT categoryID FROM tbl_product_category WHERE categoryID = '$temp'");
        if(!$res && $not_inserted) {
            $not_inserted = false;
        }
    }
    ...
    $add_sql = mysqli_query($conn, "INSERT INTO tbl_product_category (categoryID, categoryName, categoryDesc) 
                                    VALUES ('$id', '$name', '$desc')");
    ...
    if($add_sql) {
        ...
        $logContent = "Added Product Category ".$id." with the following identifications:
            Category Name: ".$name.",
            Category Description: ".$desc."";

        $log_sql = mysqli_query($conn, "INSERT INTO tbl_activity_log (logID, logName, logContent, accountID)
                                        VALUES ('$logID', 'Add Product Category', '$logContent', '".$user['accountID']."')");
...
?>
                <div class="container-fluid">
                    <h3 class="text-dark mb-4">Add New Product Category</h3>
                    <div class="row">
                        <div class="col">
                            <div class="card shadow mb-3">
                                <div class="card-header py-3">
                                    <p class="text-primary m-0 fw-bold">Product Category Information</p>
                                </div>
                                <div class="card-body">
                                    <form method="POST">
                                        <div class="row">
                                        ...
                                        <div class ="row">
                                            <div class="col-lg-2">
                                                <div class="col-mb-2">
                                                    <button class="btn btn-primary btn-sm" type="submit" name="add">Add Category</button>
                                                </div>
                                            </div>
                                        </div>
                                    </form>
